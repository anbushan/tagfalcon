...
import { fetchTrendingVideos, categoryName } from "@/lib/trends";
import { z } from "zod";
...
export async function POST(req: NextRequest) {
  ...
  const { region, categoryId, language } = parsed.data;
  ...
  const cacheKey = `trends:${region}:${categoryId || "all"}:${language || "any"}`;
  const results = await cached(cacheKey, 60 * 30, () => fetchTrendingVideos(region, categoryId, language, apiKey));
  ...
  try {
    await checkAndIncrementUsage(userId, "trendsResearchCount");
  } catch (e: any) {
  ...
  return NextResponse.json({ region, categoryName: catName, language: language || null, results });
}

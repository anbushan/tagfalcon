...
import { fetchUploadTimeData } from "@/lib/upload-time";
import { z } from "zod";
...
export async function POST(req: NextRequest) {
  ...
  const data = await fetchUploadTimeData(resolved.channelId, apiKey);
  if ("apiError" in data) {
  ...
  try {
    await checkAndIncrementUsage(userId, "uploadTimeCount");
  } catch (e: any) {
  ...
  const saved = await prisma.bestUploadTime.create({
    data: {
      userId,
      ...
      bestHourUtc: null,
      breakdownJson: { dayBreakdown: data.dayBreakdown, timeBreakdown: data.timeBreakdown },
    },
  });
